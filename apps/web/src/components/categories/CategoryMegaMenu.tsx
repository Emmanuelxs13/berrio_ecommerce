...
export default function CategoryMegaMenu() {
  ...
  return (
    ...
    >
      {/* Botón de Categorías */}
      <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-dark-300 hover:text-accent-400 rounded-xl hover:bg-dark-800/50 transition-colors">
        <svg
          className="w-5 h-5"
          fill="none"
          stroke="currentColor"
      ...
      {/* Mega Menu Dropdown */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-screen max-w-6xl bg-dark-900 border border-dark-800 rounded-xl shadow-2xl z-50 animate-fade-in">
          <div className="flex">
            {/* Panel izquierdo: Categorías principales */}
            <div className="w-64 bg-dark-900/50 border-r border-dark-800 rounded-l-xl">
              <div className="p-4">
                    ...
                    <Link
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all ${
                        activeCategory === category.id
                          ? 'bg-gradient-to-r from-accent-600/20 to-purple-600/20 border border-accent-500/30 text-accent-400 font-medium'
            ...
            <div className="flex-1 p-6 bg-dark-900/80 backdrop-blur-sm rounded-r-xl">
              {activeCategory ? (
                <div>
                  {(() => {
          ...
          <div className="bg-dark-900/50 px-6 py-3 border-t border-dark-800 rounded-b-xl">
            <Link
              href="/categories"
              className="text-sm text-accent-400 hover:text-accent-300 font-medium inline-flex items-center gap-1"
            >
